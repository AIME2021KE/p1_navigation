...
#        print('l, type s, a, r, ns, d:', type(states), type(actions), type(rewards), type(next_states), type(dones))
#        print('l, dir s, a, r, ns, d:', dir(states), dir(actions), dir(rewards), dir(next_states), dir(dones))
        qt_nextobj = self.qnetwork_target(next_states)
#        print('l, qtno:', type(qt_nextobj), dir(qt_nextobj))
        # we want a true copy of the qt_nextobj tensor, using detach to get it
        qt_nexttensor = qt_nextobj.detach()
#        print('l, qtnt:', type(qt_nexttensor), dir(qt_nexttensor))
        # next is tricky. We use max(1) to get the maximum in the tensor in dim=1 (implied?!)
        #  and we take the first value [0] incase more than one and finally
...
#        print('l, qe:', type(qexp), dir(qexp))
        ...
        # minimize loss by a) zero the gradients, prop loss backwards, and perform a step 
